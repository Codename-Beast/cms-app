...
trait PostPresenter
{
   ...
   /**
    * Set the created_by_name attribute.
    *
    * @param string $value The value to be set for created_by_name attribute.
    * @return void
    *
    * @bugfix 13.08.2024
    * @author Bernd Schreistetter
    * @Bug Attempt to read property "name" on null
    */
   public function setCreatedByNameAttribute($value)
   {
      /**
       *   if (auth()->check()) {
       *    $this->attributes['created_by_name'] = empty($value) ? auth()->user()->name : trim(label_case($value));
       *   } else {
       *    $this->attributes['created_by_name'] = null;
       *}
        */
           $this->attributes['created_by_name'] = auth()->check() ?
               empty($value) ? auth()->user()->name : trim(label_case($value)) :
               null;
   }
